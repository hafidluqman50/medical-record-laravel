...
import { DistributionMedicineDetail } from './type';
...
interface DistributionMedicineDetails {
    data:Array<DistributionMedicineDetail>;
    links:Array<{
        url?:string,
        label:string,
        active:boolean
...
type DistributionMedicineDetailProps = {
    distribution_medicine_details:DistributionMedicineDetails
    id:number
}

export default function Detail({auth, distribution_medicine_details, page_num, id}: PageProps & DistributionMedicineDetailProps) {
    ...
    return (
        <AdministratorLayout
            ...
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Data Distribusi Obat Detail</h2>}
        >
            <Head title="Data Distribusi Obat Detail" />
                        ...
                        <Table className="border-collapse border border-slate-200">
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="border border-slate-200">Lokasi Distribusi</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {
                                distribution_medicine_details.data.length == 0 ? 
                                <TableRow>
                                    <TableCell colSpan={8} align="center">
                                        Empty Data!
                                    </TableCell>
                                </TableRow>
                                : distribution_medicine_details.data.map((row, key) => (
                                    <TableRow key={row.id}>
                            ...
                            <TableRow>
                                <TableCell colSpan={8}>
                                    <Pagination>
                                        <PaginationContent>    
                                    {
                                        distribution_medicine_details.links.map((pagination: any, key: number) => (
