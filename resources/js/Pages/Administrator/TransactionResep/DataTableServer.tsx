...
export function DataTableRekamMedis() {
    ...
    const fetchMedicalRecord = async ({queryKey}: QueryFunctionContext<[
        string, 
        number
    ]>): Promise<{medical_records:MedicalRecord[], max_page:number}> => {
        const [_, pageNum] = queryKey
        const response = await axios.get<{
            data: {
                medical_records:MedicalRecord[], max_page:number
            }
    ...
    return(
            <> 
            {
                ...
                </> : openDialog == 'riwayat' ? <DataTableRekamMedisRiwayat 
                        id={medicalRecordId} 
...
export function DataTableRekamMedisRiwayat({
    id, setOpenDialog = () => {}, setMedicalRecordListId = () => {}
}: {id:number, setOpenDialog: CallableFunction, setMedicalRecordListId: CallableFunction}) {
    ...
    return(
        <>
            <Button variant="secondary" className="w-20" onClick={() => setOpenDialog('pasien')}>Kembali</Button>
            <div className="flex w-full space-x-4">
                <Input type="search" placeholder="Cari ..." onChange={(event) => setSearch(event.target.value)} />
                <Button variant="secondary" onClick={() => refetch()}>Cari</Button>
            </div>
            <Table className="border-collapse border border-slate-200">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="border border-slate-200">Tindakan Lab</TableHead>
              ...
              <TableBody>
                {
                    isLoading ?
                    <TableRow>
                        <TableCell colSpan={19}><p className="text-center">Loading ... </p></TableCell>
                    </TableRow>
                    :
                    data?.medical_record_lists.length == 0 ? 
                    <TableRow>
                        <TableCell colSpan={19}><p className="text-center">Empty Data!</p></TableCell>
                    </TableRow> :
...
export function DataTableRekamMedisDetail({
    medicalRecordId, medicalRecordListId, setOpenDialog = () => {}
}: {medicalRecordId:number, medicalRecordListId:number, setOpenDialog: CallableFunction}) {
    ...
    const fetchMedicalRecordDetail = async ({queryKey}: QueryFunctionContext<[
        string, 
        number,
        number,
        number
    ...
    const { isLoading, isError, data, error, refetch } = useQuery({
        queryKey:['medicalRecordDetails', pageNum, medicalRecordId, medicalRecordListId],
        queryFn:fetchMedicalRecordDetail
    })

    return(
        <>
            <Button variant="secondary" className="w-20" onClick={() => setOpenDialog('riwayat')}>Kembali</Button>
            <div className="flex w-full space-x-4">
                <Input type="search" placeholder="Cari ..." onChange={(event) => setSearch(event.target.value)} />
                <Button variant="secondary" onClick={() => refetch()}>Cari</Button>
            </div>
              ...
              <TableBody>
                {
                    isLoading ?
                    <TableRow>
                        <TableCell colSpan={5}><p className="text-center">Loading ... </p></TableCell>
                    </TableRow>
                    :
                    data?.medical_record_details.length == 0 ? 
                    <TableRow>
                        <TableCell colSpan={5}><p className="text-center">Empty Data!</p></TableCell>
                    </TableRow> :
