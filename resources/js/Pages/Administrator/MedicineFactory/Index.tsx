...
type MedicineFactoryProps = {
    medicine_factories:MedicineFactories
}

export default function Index({auth, app, medicine_factories, page_num}: PageProps & MedicineFactoryProps) {
    ...
    const submitDelete = (id: number): void => {
        router.delete(route('administrator.medicine-factories.delete',id))
    }
    ...
    const search = (): void => {
        router.get(
            route('administrator.medicine-factories'),
            {
                search:searchData
            },
            {
    ...
    return (
        <AdministratorLayout
            ...
            routeChild="data-pabrik-obat"
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Data Pabrik Obat</h2>}
        >
            <Head title="Data Pabrik Obat" />
                            ...
                            <div className="flex">
                                <div className="grow">
                                    <Button className="mb-2" asChild>
                                        <Link href={route('administrator.medicine-factories.create')}>Tambah Pabrik Obat</Link>
                                    </Button>
                                </div>
                            ...
                            <Table className="border-collapse border border-slate-200">
                              <TableHeader>
                                <TableRow>
                                  ...
                                  <TableHead className="border border-slate-200">Nama Pabrik</TableHead>
                              ...
                              <TableBody>
                                {
                                    medicine_factories.data.length == 0 ? 
                                    <TableRow>
                                        <TableCell colSpan={6} align="center">
                                            Empty Data!
                                        </TableCell>
                                    </TableRow>
                                    : medicine_factories.data.map((row, key) => (
                                        <TableRow key={row.id}>
                                            ...
                                            <TableCell className="border border-slate-200">
                                                <div className="flex space-x-4">
                                                    <Button className="bg-amber-500 text-white hover:bg-amber-500" asChild>
                                                        <Link href={route('administrator.medicine-factories.edit', row.id)}>Edit</Link>
                                                    </Button>
                                                    <AlertDialog>
                                ...
                                <TableRow>
                                    <TableCell colSpan={6}>
                                        <Pagination>
                                            <PaginationContent>    
                                        {
                                            medicine_factories.links.map((pagination, key) => (
