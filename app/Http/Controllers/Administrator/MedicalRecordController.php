...
class MedicalRecordController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $medical_records = MedicalRecord::with(["patient"])
            ->when($search != "", function (Builder $query) use ($search) {
                $query->whereHas("patient", function (Builder $queryHas) use (
                    $search
                ) {
        ...
        $page_num =
            ($medical_records->currentPage() - 1) *
                $medical_records->perPage() +
            1;

        return Inertia::render(
            "Administrator/MedicalRecord/Index",
            compact("medical_records", "page_num")
        );
    }
    ...
    public function create(): Response
    {
        ...
        $price_parameter = PriceParameter::where(
            "label",
            "Tunai"
        )->firstOrFail();

        $medicines = Medicine::with(["medicineFactory"])
            ->get()
            ->map(function (Medicine $medicine) {
                ...
                $medicine->capital_price_vat = format_rupiah(
                    $capital_price_vat
                );
        ...
        return Inertia::render(
            "Administrator/MedicalRecord/Create",
            compact(
                "registrations",
                "kode_transaksi",
                "price_parameter",
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            "lab_action_id" =>
                "required|integer|exists:" . LabAction::class . ",id",
        ...
        try {
            $get_registration_by_id = Registration::where(
                "id",
                $request->registration_id
            )->firstOrFail();

            $check_medical_record = MedicalRecord::where(
                "patient_id",
                $get_registration_by_id->patient_id
            )->exists();

            if ($check_medical_record) {
                $medical_record_id = MedicalRecord::where(
                    "patient_id",
                    $get_registration_by_id->patient_id
                )->firstOrFail()->id;
            } else {
            ...
            $input_medical_record_list[
                "medical_record_id"
            ] = $medical_record_id;
            $input_medical_record_list["main_complaint"] =
                $input_medical_record_list["complains_of_pain"];
            ...
            $medical_record_list_id = MedicalRecordList::insertGetId(
                $input_medical_record_list
            );
                ...
                foreach ($prescription_name as $key => $value) {
                    ...
                    foreach ($medicines as $k => $v) {
                        if ($value["prefixNum"] == $v["prefixNum"]) {
                            ...
                            PrescriptionList::where("id", $prescription_list_id)
                                ->where("prescription_id", $prescription_id)
                                ->increment("service_fee", (int) $v["jasa"]);
                            PrescriptionList::where("id", $prescription_list_id)
                                ->where("prescription_id", $prescription_id)
                                ->increment("total_costs", (int) $v["total"]);
                            PrescriptionList::where("id", $prescription_list_id)
                                ->where("prescription_id", $prescription_id)
                                ->increment(
                                    "total_prescription_packs",
                                    (int) $v["prescription_packs"]
                                );
                        }
                    }
                }
            ...
            return redirect()
                ->intended("/administrator/medical-records")
                ->with("success", "Berhasil Input Rekam Medis!");
        } catch (Exception $e) {
            DB::rollBack();
    ...
    public function listRecords(
        ...
        $medical_record_lists = MedicalRecordList::with([
            ...
            ->when($search != "", function (Builder $query) use ($search) {
                $query
                    ->whereHas("registration.patient", function (
                        Builder $queryHas
                    ) use ($search) {
                        $queryHas->where("name", "like", "%{$search}%");
                    })
                    ->orWhereHas("registration.doctor", function (
                        Builder $queryHas
                    ) use ($search) {
                        $queryHas->where("name", "like", "%{$search}%");
                    });
        ...
        $page_num =
            ($medical_record_lists->currentPage() - 1) *
                $medical_record_lists->perPage() +
            1;

        return Inertia::render(
            "Administrator/MedicalRecord/ListRecord",
            compact("medical_record_lists", "page_num")
        );
    }
    ...
    public function detailRecords(
        Request $request,
        int $medical_record_id,
        int $medical_record_list_id
    ): Response {
        $search = $request->search;

        $medical_record_details = MedicalRecordDetail::with([
            "medicalRecordList",
            "medicine",
        ])
            ->when($search != "", function (Builder $query) use ($search) {
                $query->whereHas("medicine", function (Builder $queryHas) use (
                    $search
                ) {
                    $queryHas->where("name", "like", "%{$search}%");
                });
            })
            ->whereHas("medicalRecordList", function (Builder $queryHas) use (
                $medical_record_id
            ) {
        ...
        $page_num =
            ($medical_record_details->currentPage() - 1) *
                $medical_record_details->perPage() +
            1;

        return Inertia::render(
            "Administrator/MedicalRecord/DetailRecord",
            compact(
                "medical_record_id",
    ...
    public function delete(int $id): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()
                ->intended("/administrator/medical-records")
                ->with("success", "Berhasil Hapus Rekam Medis!");
        } catch (Exception $e) {
            DB::rollBack();
